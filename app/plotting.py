...
def clusters_chart(
    df: pd.DataFrame,
    title_col: str = "cluster_center",
    hover_col: str = "cluster_center",
...
def plot_distance_histogram(max_distance, mean_distance):
    ...
    fig = px.histogram(max_distance[max_distance > 0.01], nbins=50, width=500, height=300, title="Maximum distance")
    st.plotly_chart(fig)

    fig = px.histogram(mean_distance, nbins=50, width=500, height=300, title="Mean distance")
    st.plotly_chart(fig)
...
def plot_size_histogram(cluster_size: np.array) -> None:
    ...
    """
    fig = px.histogram(x=cluster_size, nbins=50, width=500, height=300)
    st.plotly_chart(fig)
...
def plot_dist_matrix(embeddings: np.array, titles, width: int = 800, height: int = 600):
    ...
    fig = go.Figure(data=go.Heatmap(z=dist_matrix, x=titles, y=titles, colorscale="Viridis_r"))

    fig.update_layout(
        autosize=False, width=width, height=height, xaxis_showticklabels=False, yaxis_showticklabels=False
    )
